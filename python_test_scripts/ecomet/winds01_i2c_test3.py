...
def pad_to_20_left(bytes_list: list[int]) -> list[int]:
    """Strip leading 0x00 then pad left with 0x00 to exactly 19 bytes"""
    # Convert to bytes and strip leading zeros
    data = bytes(bytes_list).lstrip(b'\x00')
    # Pad left
    padding_needed = 20 - len(data)
    if padding_needed < 0:
...
while (1):
	...
	#time.sleep(stime)
	data_bulk = pad_to_20_left(int_to_hex_bytes(sens.read_register ( register = 'REG_BULK' )[0]))
	#data_bulk = bytes.fromhex(str(sens.read_register ( register = 'REG_BULK' )[0]))
	...
	#print (f"AVG: {round(avg00,2)} m.s, GUST: {round(gust00,2)} m.s SET: {set}")
	print (f"Count: {reg_ValidCnt} AVG: {round(avg00,2)} m.s, GUST: {round(gust00,2)} m.s SET: {set}")
	if set >= 3:
		if set == 3:
